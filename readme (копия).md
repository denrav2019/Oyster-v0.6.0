...
##### `strcmp(s1, s2)` — Сравнивает две строки и возвращает:(-1 если s1 < s2, 0 если s1 == s2, 1 если s1 > s2) Поддерживает Unicode.
```oyster
$cmp = strcmp("abc", "abd")   # -1
$cmp = strcmp("abc", "abc")   # 0
$cmp = strcmp("abd", "abc")   # 1
```
##### `setstr(buf, offset, str)` — Побайтово копирует строку str в буфер buf начиная с позиции offset. Строка не должна превышать размер буфера.
```oyster
$buf = bytearray(64)
setstr($buf, 20, "Oyster")   # записать "Oyster" в буфер начиная с 20-го байта
```

##### `getstr(buf, offset, maxlen)` — Читает строку из буфера buf начиная с позиции offset.
Читает до maxlen байт или до нулевого байта (конец строки).
```oyster
$name = getstr($buf, 20, 44)  # прочитать до 44 байт начиная с 20-го байта
```

##### `writestr(fh, str)` — Записывает строку в файл в бинарном формате: сначала 4 байта длины (little-endian int32), затем сама строка. 
```oyster
...
##### `bytearray(n)` — Cоздание бинарного буфера - строка ASCII длиной n, заполненная символом "\0"
...
##### `readstr(fh)` — Читает строку из файла в бинарном формате: сначала 4 байта длины, затем сама строка. 
Возвращает прочитанную строку или пустую строку если не удалось прочитать длину.
```oyster
$str = readstr($fh)    # читает длину, потом строку
```

##### `gets()` — Чтение строки со стандартного ввода (читает одну строку из стандартного ввода (до \n). 
...
##### `get8(var, index)` — Прочитать 1 байт. 
...
##### `get16(var, index)` — Прочитать 2 байта. 
...
##### `get32(var, index)` — Прочитать 4 байта. 
...
```
##### `get64(var, index)` — Прочитать 8 байт. 
...
```
##### `set8(var, index, value)` — Записать 1 байт. 
...
```
##### `set16(var, index, value)` — Записать 2 байта. 
...
$x = set16($buf, 0, 0x1234)
```
##### `set32(var, index, value)` — Записать 4 байта. 
...
```
##### `set64(var, index, value)` — Записать 8 байт. 
...
```
Пример: формирование заголовка записи


##### Создать запись класса: [id(8)][parent_id(8)][name_len(4)][name(44)]
```oyster
$buf = bytearray(64)
$x = set64($buf, 0, $id)
$x = set64($buf, 8, $parent_id)
$x = set32($buf, 16, len($name))
setstr($buf, 20, $name)
```
##### Записать в файл
```oyster
...
##### `reverse(@arr)` — Перевернуть массив
...
##### socket(domain, type, protocol)    # Создать сокет. Создаёт сокет и возвращает его дескриптор (целое число). При ошибке возвращает `-1`.
Аргументы:
...
##### connect(sock, host, port)         # Подключиться к серверу. 
...
##### bind(sock, host, port)            # Привязать сокет к адресу. 
...
##### listen(sock, backlog)             # Слушать входящие соединения. 
...
##### accept(sock)                      # Принять входящее соединение. 
...
Что добавить в README для 0.6.0:
Новые возможности:
Раздельные таблицы глобальных и локальных переменных

my — только внутри функций

d/q — опкоды для локальных

v/p — для глобальных

Модуль OysterDB (db.osm)

Объектная БД с классами, свойствами, объектами

db_open, db_insert, db_select, db_search, db_multysearch

Последовательные порты

serial_open, serial_read, serial_close

Процессы и pipe

popen2, pipe_read, pipe_write, pipe_close
...
round($x, $n, $mode) — округление

popen, pclose — односторонний pipe

Исправления

Точный вывод дробных чисел в print

Кэш модулей (без повторной компиляции)

Хочешь, чтобы я написал готовый README.md для 0.6.0?
Или ты сам обновишь — я помогу с конкретными разделами.

сам обновлю, помоги с тем, что было изменено. желательно в том стиле, какой в исходном файле
Вот изменения для README в стиле исходника:

Раздельные таблицы переменных
Локальные переменные объявляются только через my внутри функций. Глобальные — в основном коде.
...
Опкоды: v/p — глобальные, d/q — локальные.

Модуль OysterDB
Объектная БД с классами, свойствами и объектами.

oyster
use "db" as db

%db = db.db_open("./mydb")
$class = db.db_create_class(%db, "item", 0)
$x = db.db_add_property(%db, $class, "name", 1)
$id = db.db_insert(%db, $class, ("name" => "Milk"))
%obj = db.db_select(%db, $id)
print(%obj["name"])
Последовательные порты
oyster
...
pipe_close($proc)
Графический интерфейс
Модули tk.osm и ui.osm — обёртки над Tcl/Tk.
...
$tk = tk.tk_init()
ui.ui_window($tk, "My App", 800, 600)
ui.ui_button($tk, 1, "Click", "CLICKED")
$event = ui.ui_wait_event($tk, 3000)
Функция round
oyster
round(1.05, 1, 1)  # 1.1
round(1.05, 1, 2)  # 1.0
